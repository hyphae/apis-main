...
public class MediatorRequestHandling extends AbstractVerticle {
	...
	private void startMediatorRequestHandlingService_(Handler<AsyncResult<Void>> onComplete) {
	...
	private void doHandleMediatorRequest_(JsonObject request, Handler<AsyncResult<JsonObject>> onComplete) {
						...
						StateHandling.operationMode(vertx, resOperationMode -> {
							if (resOperationMode.succeeded()) {
								...
								if ("autonomous".equals(operationMode)) {
									doHandleMediatorRequest__(request, onComplete);
								} else {
									if (log.isInfoEnabled())
	...
	private void doHandleMediatorRequest__(JsonObject request, Handler<AsyncResult<JsonObject>> onComplete) {
													...
													if (repBatteryCapacityTest.succeeded()) {
														if (repBatteryCapacityTest.result().body()) {
															...
															if (efficientGridVoltageV != null) {
																accept.put("efficientGridVoltageV",
