...
public class DealActivation extends AbstractDealExecution {
	...
	protected void doExecute(Handler<AsyncResult<Void>> onComplete) {
		...
		if (dealValidationError == null) {
			...
			if (canActivate_()) {
									...
									if (resActivateDeal.succeeded()) {
										...
										moveMasterDeal_(resMoveMasterDeal -> {
											if (resMoveMasterDeal.succeeded()) {
												if (isFirstDeal_) {
													// If this is the first interchange, proceed to the voltage ramp-up
													// process
													// 最初の融通なら電圧ランプアップ処理に移行する
													new DealRampingUp(this).execute(onComplete);
												} else {
													// For the second and subsequent interchanges, proceed to the
										...
										onComplete.handle(resActivateDeal);
									}
								});
							} else {
								onComplete.handle(resActivateDcdc);
							}
						});
					} else {
				...
				// Do nothing unless conditions are right for attaching the unit on the voltage
				// reference side
			...
			// 捨てる場合は融通破棄処理に移行する
			new DealDispositionWithAbortReason(this, dealValidationError).execute(onComplete);
		}
	}
	 ...
	 * 
	 * @return If NG, a character string explaining the reason why, or {@code null}
	 *         if accepted
	 * 
	 ...
	 * 
	 * @return an error string if the limit has been exceeded, or {@code null} if
	 *         accepted
	 * 
	...
	private String checkNumberOfResets_() {
		...
		if (resetLimit_ != null) {
			...
			String msg = "data deficiency; POLICY.gridMaster.deal.resetLimit : "
					+ PolicyKeeping.cache().getValue("gridMaster", "deal", "resetLimit");
		...
		for (JsonObject aDeal : otherDeals_) {
			...
			if (Deal.masterSideUnitMustBeActive(aDeal)) {
				...
				if (Deal.isChargeUnit(aDeal, dischargeUnitId_)) {
					...
					String msg = "other charge side deal found on discharge unit : " + dischargeUnitId_
							+ " ; other deal : " + aDeal;
				...
				if (Deal.isDischargeUnit(aDeal, chargeUnitId_)) {
					...
					String msg = "other discharge side deal found on charge unit : " + chargeUnitId_
							+ " ; other deal : " + aDeal;
					if (log.isInfoEnabled())
						log.info(msg);
					return msg;
	 ...
	 * Check the current capacity.
	 * Return an error string if NG.
	 * Judgement is made based on
	 * {@link GridBranchCurrentCapacity#checkNewDeal(Vertx, JsonObject, JsonObject, List)}.
	 * 
		...
		if (gridVoltageV != null) {
			...
			if (minOperationGridVoltageV != null && gridVoltageSeparationV != null && gridUvloMaskV != null) {
				...
				if (gridVoltageV <= gridVoltageMaxV) {
					// Present grid voltage is no larger than POLICY.operationGridVoltageVRange.min
					// + POLICY.gridVoltageSeparationV × 2
					// 現在のグリッド電圧が POLICY.operationGridVoltageVRange.min +
					// POLICY.gridVoltageSeparationV x 2 以下で
					...
					if (gridVoltageV < maskMinV || maskMaxV < gridVoltageV) {
						// OK if less than POLICY.operationGridVoltageVRange.min - POLICY.gridUvloMaskV
						// or greater than POLICY.operationGridVoltageVRange.min + POLICY.gridUvloMaskV
						// POLICY.operationGridVoltageVRange.min - POLICY.gridUvloMaskV 未満 または
						// POLICY.operationGridVoltageVRange.min + POLICY.gridUvloMaskV を超えていたら OK
						return true;
					} else {
						// NG if in the range from POLICY.operationGridVoltageVRange.min -
						// POLICY.gridUvloMaskV to POLICY.operationGridVoltageVRange.min +
						// POLICY.gridUvloMaskV
						// POLICY.operationGridVoltageVRange.min - POLICY.gridUvloMaskV 以上
						// POLICY.operationGridVoltageVRange.min + POLICY.gridUvloMaskV 以下なら NG
						if (log.isInfoEnabled())
							log.info("can not start voltage reference yet ( grid voltage : " + gridVoltageV
									+ ", should be lower than : " + maskMinV + " or greater than : " + maskMaxV
									+ " ) ...");
					}
				} else {
					// GLOBAL ERROR if the present grid voltage exceeds
					// POLICY.operationGridVoltageVRange.min + POLICY.gridVoltageSeparationV × 2
					// 現在のグリッド電圧が POLICY.operationGridVoltageVRange.min +
					// POLICY.gridVoltageSeparationV x 2 を超えていたら GLOBAL ERROR
					ErrorUtil.report(vertx_, Error.Category.HARDWARE, Error.Extent.GLOBAL, Error.Level.ERROR,
							"invalid grid voltage : " + gridVoltageV + ", should be lower than or equal to : "
									+ gridVoltageMaxV);
				}
			} else {
				ErrorUtil.report(vertx_, Error.Category.USER, Error.Extent.LOCAL, Error.Level.ERROR,
						"data deficiency ; POLICY.operationGridVoltageVRange.min : " + minOperationGridVoltageV
								+ "POLICY.gridVoltageSeparationV : " + gridVoltageSeparationV
								+ ", POLICY.gridUvloMaskV : " + gridUvloMaskV);
			}
		} else {
			ErrorUtil.report(vertx_, Error.Category.LOGIC, Error.Extent.GLOBAL, Error.Level.WARN,
					"no dcdc.meter.vg value in master side unit data : " + masterSideUnitData_());
		}
		return false;
	}
	...
	private boolean canActivateNonFirstDeal_() {
		for (JsonObject aDeal : otherDeals_) {
			if (Deal.isStarted(aDeal) && !Deal.isDeactivated(aDeal)) {
				// OK if there is at least one other interchange that has been started and not
				// deactivated
		...
		if (log.isInfoEnabled())
			log.info("can not start master side unit yet ( other deal should be ramping up ) ...");
		// NG if there are none --- either this is the first interchange or voltage
		// ramp-up is still in progress
		// 一つもなければ「最初の融通がまだ電圧ランプアップ中」のはずなので NG
		return false;
	}

	private void prepareActivate_(Handler<AsyncResult<Void>> onComplete) {
		// Update the control state of the voltage reference to record the data required
		...
		prepareVoltageReference_(resPrepareVoltageReference -> {
			if (resPrepareVoltageReference.succeeded()) {
				// Perform judgments and preparations necessary for switching around the voltage
				// reference side, moving the voltage reference, etc.
				...
				if (willMoveVoltageReference_) {
					...
					if ("theoretical".equals(voltageReferenceTakeOverDvg)) {
						...
						operationGridVoltageV_ = DealExecution.unitDataCache.getFloat(currentVoltageReferenceUnitId_,
								"dcdc", "vdis", "dvg");
						if (operationGridVoltageV_ == null) {
							ErrorUtil.reportAndFail(vertx_, Error.Category.LOGIC, Error.Extent.GLOBAL, Error.Level.WARN,
									"no dcdc.vdis.dvg value in voltage reference unit data : "
											+ DealExecution.unitDataCache.getJsonObject(currentVoltageReferenceUnitId_),
									onComplete);
							return;
						}
					}
				...
				if (dealGridCurrentA_ != null) {
					if (isFirstDeal_ || (willMoveVoltageReference_ && DDCon.Mode.WAIT == masterSideUnitDDConMode_())) {
						// When the first interchange or voltage reference movement occurs, and the
						// voltage reference side is in a WAIT state (i.e., WAIT → VR control)
						...
						// The current compensation target value is the interchange current itself (the
						// negative sign differs between the discharging and charging sides)
						// 電流コンペンセイションのターゲット値は融通電流そのもの ( 送電側か受電側で負号は異なる )
						deal_.put("compensationTargetVoltageReferenceGridCurrentA", result);
						if (log.isInfoEnabled())
							log.info("compensationTargetVoltageReferenceGridCurrentA ( +/- dealGridCurrentA ) : "
									+ result);
						onComplete.handle(Future.succeededFuture());
					} else {
						// If this is neither the first interchange nor subject to WAIT → VR control
						// 最初の融通ではなく WAIT → VR の制御でもない場合
						final String targetVoltageReferenceUnitId = (willMoveVoltageReference_) ? masterSideUnitId_()
								: currentVoltageReferenceUnitId_;
						if (targetVoltageReferenceUnitId != null) {
							if (targetVoltageReferenceUnitId.equals(masterSideUnitId_())) {
								...
								// 電圧リファレンスが移動してくる場合
								Float masterSideGridCurrentA = JsonObjectUtil.getFloat(masterSideUnitData_(), "dcdc",
										"meter", "ig");
								if (masterSideGridCurrentA != null) {
									...
									// The current compensation target value is obtained by adding the present ig
									// value of the unit on the voltage reference side of this interchange to the
									// interchange current (the negative sign differs between the discharging and
									// charging sides)
									// 電流コンペンセイションのターゲット値は当該融通の電圧リファレンス側ユニットの現在の ig 値に融通電流 ( 送電側か受電側で負号は異なる )
									// を加算したもの
									deal_.put("compensationTargetVoltageReferenceGridCurrentA", result);
									if (log.isInfoEnabled())
										log.info(
												"compensationTargetVoltageReferenceGridCurrentA ( masterSideGridCurrentA +/- dealGridCurrentA ) : "
														+ masterSideGridCurrentA + " , " + dealGridCurrentA_ + " , "
														+ result);
									onComplete.handle(Future.succeededFuture());
								} else {
									ErrorUtil.reportAndFail(vertx_, Error.Category.LOGIC, Error.Extent.GLOBAL,
											Error.Level.WARN, "no dcdc.meter.ig value in master side unit data : "
													+ masterSideUnitData_(),
											onComplete);
								}
							} else {
								...
								// 電圧リファレンスが移動してこない場合
								Float targetVoltageReferenceUnitGridCurrentA = DealExecution.unitDataCache
										.getFloat(targetVoltageReferenceUnitId, "dcdc", "meter", "ig");
								if (targetVoltageReferenceUnitGridCurrentA != null) {
									float result = targetVoltageReferenceUnitGridCurrentA;
									// The current compensation target value is just the present ig value of the
									// present voltage reference unit
									// 電流コンペンセイションのターゲット値は現在の電圧リファレンスユニットの現在の ig 値そのもの
									deal_.put("compensationTargetVoltageReferenceGridCurrentA", result);
									if (log.isInfoEnabled())
										log.info(
												"compensationTargetVoltageReferenceGridCurrentA ( targetVoltageReferenceUnitGridCurrentA ) : "
														+ result);
									onComplete.handle(Future.succeededFuture());
								} else {
									ErrorUtil.reportAndFail(vertx_, Error.Category.LOGIC, Error.Extent.GLOBAL,
											...
											"no dcdc.meter.ig value in voltage reference unit data : "
													+ DealExecution.unitDataCache
															.getJsonObject(targetVoltageReferenceUnitId),
											onComplete);
								}
							}
						} else {
							ErrorUtil.reportAndFail(vertx_, Error.Category.LOGIC, Error.Extent.GLOBAL,
									Error.Level.ERROR, "no voltage reference found", onComplete);
						}
					}
				} else {
					ErrorUtil.reportAndFail(vertx_, Error.Category.LOGIC, Error.Extent.GLOBAL, Error.Level.WARN,
				...
				onComplete.handle(resPrepareVoltageReference);
			}
		});
	}

	private void prepareVoltageReference_(Handler<AsyncResult<Void>> onComplete) {
		...
		if (currentVoltageReferenceUnitId_ != null) {
			...
			if (log.isInfoEnabled())
				log.info("voltage reference unit : " + currentVoltageReferenceUnitId_);
			if (Deal.isInvolved(deal_, currentVoltageReferenceUnitId_)) {
				...
				if (log.isDebugEnabled())
					log.debug("no need to update voltage reference unit status");
				// No need to do anything because the data of both units was updated at the
				// start of interchange processing
				...
				if (log.isDebugEnabled())
					log.debug("update voltage reference unit status...");
				// Update the cached device control state of the voltage reference unit
				// 電圧リファレンスユニットのデバイス制御状態のキャッシュを更新しておく
				updateUnitDcdcStatus_(currentVoltageReferenceUnitId_, onComplete);
			}
		} else {
		...
		if (largeCapacityUnitIds != null && !largeCapacityUnitIds.isEmpty()) {
			...
			if (isFirstDeal_) {
				...
				if (!largeCapacityUnitIds.contains(masterSideUnitId_())
						&& largeCapacityUnitIds.contains(slaveSideUnitId_())) {
					...
					if (tryToFlipMasterSide_()) {
						if (log.isInfoEnabled())
							log.info("master side was flipped : " + masterSide_);
					} else {
						if (log.isWarnEnabled())
							log.warn("could not flip master side");
					}
				}
			} else {
				...
				// 二番目以降の融通の場合
				if (currentVoltageReferenceUnitId_ != null
						&& !largeCapacityUnitIds.contains(currentVoltageReferenceUnitId_)) {
					// If the present voltage reference is not a large capacity unit
					// 現在の電圧リファレンスが大容量ユニットではない場合
					if (log.isInfoEnabled())
						log.info("voltage reference unit is not a large capacity unit");
					if (largeCapacityUnitIds.contains(masterSideUnitId_())) {
						// If the unit on the voltage reference side of this interchange is a large
						// capacity unit
						...
						if (log.isInfoEnabled())
							log.info("will move voltage reference");
					} else if (largeCapacityUnitIds.contains(slaveSideUnitId_())) {
						// If the unit on the voltage reference side of this interchange is not a large
						// capacity unit
					...
					// If there is not yet any voltage reference, or if the present voltage
					// reference is a large capacity unit,
					// まだ電圧リファレンスが存在しない, もしくは現在の電圧リファレンスが大容量ユニットである場合
					// do nothing
					// なにもしない
					if (log.isInfoEnabled())
						log.info("no voltage reference unit or voltage reference unit is a large capacity unit");
				}
				if (willMoveVoltageReference_) {
					// If the voltage reference is to be moved
					...
					// Decide on a new mode for the present voltage reference (i.e. the mode to
					// apply after moving the voltage reference)
					...
					for (JsonObject aDeal : otherDeals_(currentVoltageReferenceUnitId_)) {
						...
						if (oldVoltageReferenceNewMode_ == null || Deal.slaveSideUnitMustBeActive(aDeal)) {
							// If there is no candidate yet, or if the unit on the non-voltage-reference
							// side is active (the result is the same, but may occur when the units at both
							// ends are active)
							// まだ候補が出ていない若くは電圧リファレンスでない側が点いている場合 ( これ結果は同じだけど "両端ユニットが点いている場合" なのかもしれない )
							// → Prioritize interchanges for which control has already started, but if there
							// are none, begin with an interchange that has not yet started
							// → もう制御が始まっている融通が優先だけど無ければそうでない融通から判定する ということ
							if (Deal.isDischargeUnit(aDeal, currentVoltageReferenceUnitId_)) {
								// DISCHARGE if the present voltage reference is the unit on the discharging
								// side
					...
					Float currentVoltageReferenceIg = DealExecution.unitDataCache
							.getFloat(currentVoltageReferenceUnitId_, "dcdc", "meter", "ig");
					if (oldVoltageReferenceNewMode_ != null && currentVoltageReferenceIg != null) {
						// If a new mode is determined for the present voltage reference, record the
						// grid current readings for this unit
						// 現在の電圧リファレンスの新しいモードが決まっていたらそのユニットでのグリッド電流測定値を記録しておく
						oldVoltageReferenceGridCurrentA_ = currentVoltageReferenceIg;
						if (log.isInfoEnabled())
							log.info("old voltage reference will be : " + oldVoltageReferenceNewMode_ + " ( "
									+ oldVoltageReferenceGridCurrentA_ + " )");
					} else {
						if (log.isWarnEnabled())
							log.warn("failed to move voltage reference ; oldVoltageReferenceNewMode_ : "
									+ oldVoltageReferenceNewMode_ + ", oldVoltageReferenceGridCurrentA_ : "
									+ oldVoltageReferenceGridCurrentA_);
					}
				}
			}
	...
	private void prepareMode_() {
		...
		if (isFirstDeal_ || willMoveVoltageReference_) {
			// VOLTAGE_REFERENCE if this is the first interchange or the voltage reference
			// has to be moved
			...
			if (DDCon.Mode.WAIT == oldMode_) {
				...
				// 現在 WAIT なら
				// DISCHARGE if this is the discharging side, or CHARGE if this is the charging
				// side
		...
		if (log.isInfoEnabled())
			log.info("old mode : " + oldMode_ + ", new mode : " + newMode_);
	}

	private void activateDcdc_(Handler<AsyncResult<Void>> onComplete) {
		// Fail if dcdc/failBeforeActivate = true in a DEAL object
		// DEAL オブジェクト中に dcdc/failBeforeActivate = true があったら fail させる
		if (testFeature_failIfNeed_("dcdc", "failBeforeActivate", onComplete))
			return;
		if (willMoveVoltageReference_) {
			...
			controlDcdc_(currentVoltageReferenceUnitId_, "voltageReferenceWillHandOver", null, resWillHandOver -> {
				if (resWillHandOver.succeeded()) {
					// Set up operationGridVoltageV_ determined in prepareActivate_() as a new dvg
					// in the parameters...
					...
					controlMasterSideUnitDcdc_("voltageReferenceWillTakeOver", params, resWillTakeOver -> {
						if (resWillTakeOver.succeeded()) {
							if (log.isInfoEnabled())
								log.info("new voltage reference started, retire old voltage reference");
							// Set up oldVoltageReferenceNewMode_ and oldVoltageReferenceGridCurrentA_
							// determined in prepareActivate_() as parameters...
							// prepareActivate_() で決めた oldVoltageReferenceNewMode_ と
							// oldVoltageReferenceGridCurrentA_ をパラメタに仕込んで...
							params.clear().put("mode", oldVoltageReferenceNewMode_.name()).put("gridCurrentA",
									Math.abs(oldVoltageReferenceGridCurrentA_));
							// Set the destination unit to voltageReferenceDidHandOver
							// 移動元ユニットに voltageReferenceDidHandOver
							controlDcdc_(currentVoltageReferenceUnitId_, "voltageReferenceDidHandOver", params,
									resDidHandOver -> {
										if (resDidHandOver.succeeded()) {
											...
											controlMasterSideUnitDcdc_("voltageReferenceDidTakeOver", null,
													res -> testFeature_failIfNeed_(res, "dcdc", "failAfterActivate",
															onComplete));
										} else {
			...
			if (DDCon.Mode.VOLTAGE_REFERENCE == oldMode_) {
				...
				if (testFeature_failIfNeed_("dcdc", "failAfterActivate", onComplete))
					return;
				// No need to do anything
				// 何もする必要なし
				onComplete.handle(Future.succeededFuture());
				...
				// 元が WAIT だった場合
				if (log.isInfoEnabled())
					log.info("start master side unit");
				...
				// Command the unit on the voltage reference side to switch to newMode_ as
				// determined by prepareMode_()
				...
				controlMasterSideUnitDcdc_(newMode_.name(), params,
						res -> testFeature_failIfNeed_(res, "dcdc", "failAfterActivate", onComplete));
			} else {
				// If this is neither a voltage reference nor in WAIT mode (should be DISCHARGE
				// or CHARGE)
				...
				// Issue command to change the current value in the unit on the voltage
				// reference side
	...
	private void moveMasterDeal_(Handler<AsyncResult<Void>> onComplete) {
		if (willMoveVoltageReference_) {
			...
			DealUtil.isMaster(vertx_, currentMasterDeal_, false,
	...
	private void activateDeal_(Handler<AsyncResult<Void>> onComplete) {
		DealUtil.activate(vertx_, deal_, referenceDateTimeString_(), resActivateDeal -> {
			if (resActivateDeal.succeeded()) {
				...
				DealUtil.isMaster(vertx_, deal_, isFirstDeal_ || willMoveVoltageReference_,
						resIsMaster -> ErrorExceptionUtil.reportIfNeedAndHandle(vertx_, resIsMaster, onComplete));
			} else {
				ErrorExceptionUtil.reportIfNeedAndFail(vertx_, resActivateDeal.cause(), onComplete);
			}
		});
	}
