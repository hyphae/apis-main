	...
	private static void check_topologyBased_(Vertx vertx, JsonObject policy, JsonObject unitData, Handler<AsyncResult<Void>> completionHandler) {
		...
		if (branchIds != null) {
			...
			for (String aBranchId : branchIds) {
				...
				if (capacity != null && forwardUnitIds != null) {
					...
					if (Math.abs(capacity) < Math.abs(sum)) {
						errorMessages.add("branch : " + aBranchId + ", forward sum of dcdc.meter.ig : " + sum + ", exceeds capacity : " + capacity);
					}
				} else {
			...
			if (!errorMessages.isEmpty()) {
				ErrorUtil.reportAndFail(vertx, Error.Category.HARDWARE, Error.Extent.GLOBAL, Error.Level.ERROR, String.join(" ; ", errorMessages), completionHandler);
				return;
			}
			completionHandler.handle(Future.succeededFuture());
		} else {
