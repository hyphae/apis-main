...
public class DataCollection extends AbstractVerticle {
	 ...
	 * Independent of
	 * {@link jp.co.sony.csl.dcoes.apis.main.app.gridmaster.main_loop.DealExecution#unitDataCache}
	 * 
	...
	private Promise<Void> stopPromise_ = Promise.promise();
	{
		stopPromise_.complete();
	}

	/**
	 * Called at startup.
	 * Launches the {@link io.vertx.core.eventbus.EventBus} service.
	 * Launches a timer that periodically updates the cache by collecting unit data
	 * from all units.
	 * 
	 ...
	 * Use {@link #stopFuture_} to perform exclusion control so that
	 * {@link #stop(Future)} does not complete during propagation of the
	 * {@link io.vertx.core.eventbus.EventBus} service created during the process of
	 * periodically collecting unit data from all units.
	 * 
	 ...
	 *                   停止時に呼び出される.
	 *                   タイマを止めるためのフラグを立てる.
	 *                   定期的に全ユニットのユニットデータを収集する処理中に作成する
	 *                   {@link io.vertx.core.eventbus.EventBus} サービスの伝搬中に
	 *                   {@link #stop(Future)} が完了しないよう {@link #stopFuture_}
	 *                   を使って排他制御する.
	 * @param stopFuture {@inheritDoc}
	 * @throws Exception {@inheritDoc}
	 */
	...
	public void stop(Promise<Void> stopPromise) throws Exception {
		stopped_ = true;
		Promise<Void> p = this.stopPromise_;
	 ...
	 * Return the results collected by querying all the units instead of using the
	 * cached values that are periodically refreshed by GridMaster.
	 * If a timestamp was sent in the message body and the most recent data
	 * collection took place after this time, skip the data collection and return
	 * the cached value instead.
	 * Since "urgent" was not specified in the header at the time of collection,
	 * collect the cached data that has been periodically refreshed for each unit.
	 * Message body: A timestamp indicating the time after which the collection of
	 * fresh data is not required [{@link Number}]
	 * Message header: none
	 ...
	 * @param onComplete the completion handler
	 */
	private void startInternalUnitDatasService_(Handler<AsyncResult<Void>> onComplete) {
		vertx.eventBus().<Number>localConsumer(ServiceAddress.GridMaster.urgentUnitDatas(), req -> {
			...
			if (!cache.isEmpty() && noNeedToRefreshIfNewerThan != null
					&& noNeedToRefreshIfNewerThan.longValue() < lastDataCollectionMillis_) {
				// When some sort of timestamp is provided
				// 何やら時刻が送られてきた場合
				// If the cache is newer than this timestamp, return the cache without
				// collecting new data.
				// その時刻よりキャッシュ時刻の方が新しければ新たにデータ収集することなくキャッシュを返す
				if (log.isDebugEnabled())
					log.debug("no need to refresh ; noNeedToRefreshIfNewerThan : " + noNeedToRefreshIfNewerThan
							+ " , lastDataCollectionMillis_ : " + lastDataCollectionMillis_);
	 ...
	 * The timeout duration is {@code POLICY.gridMaster.dataCollectionPeriodMsec}
	 * (default: {@link #DEFAULT_DATA_COLLECTION_PERIOD_MSEC}).
	 * 
	 * データ収集タイマ設定.
	 * 待ち時間は {@code POLICY.gridMaster.dataCollectionPeriodMsec} ( デフォルト値
	 * {@link #DEFAULT_DATA_COLLECTION_PERIOD_MSEC} ).
	 */
	private void setDataCollectionTimer_() {
		Long delay = PolicyKeeping.cache().getLong(DEFAULT_DATA_COLLECTION_PERIOD_MSEC, "gridMaster",
	...
	private void dataCollectionTimerHandler_(Long timerId) {
		...
		if (null == timerId || timerId.longValue() != dataCollectionTimerId_) {
			ErrorUtil.report(vertx, Error.Category.LOGIC, Error.Extent.LOCAL, Error.Level.WARN,
					"illegal timerId : " + timerId + ", dataCollectionTimerId_ : " + dataCollectionTimerId_);
			return;
		}
		...
		if (lastDataCollectionMillis_ != 0L) {
			...
			Long period = PolicyKeeping.cache().getLong(DEFAULT_DATA_COLLECTION_PERIOD_MSEC, "gridMaster",
					"dataCollectionPeriodMsec");
			if (millisAfterLastDataCollection < period) {
				// If the timer cycle time has not elapsed since the last data collection, reset
				// the timer to the time difference and finish without doing anything
	...
	private void getDatas_(Handler<AsyncResult<JsonObject>> onComplete) {
		...
		new DataCollection_().execute_(res -> {
			if (res.succeeded()) {
				// When data collection is finished, formally remember the "starting time" that
				// was temporarily stored earlier
				...
				if (log.isInfoEnabled())
					log.info(res.result().size() + " unit data collected");
			}
	...
	private class DataCollection_ {
		...
		private void execute_(Handler<AsyncResult<JsonObject>> onComplete) {
			...
			// Set up a minor fix (stopFuture_) to ensure that the Verticle body does not
			// stop during address expansion.
			// アドレス展開中に Verticle 本体が stop するのを防ぐための小細工 ( stopFuture_ ) を仕込む
			Promise<Void> registerPromise = Promise.promise();
			stopPromise_ = registerPromise;
			// if (log.isInfoEnabled()) log.info("DataCollection:" + replyAddress + "
			// begin");
			...
			dataConsumer.handler(rep -> {
				...
				if (!isMember) {
					ErrorUtil.report(vertx, Error.Category.LOGIC, Error.Extent.LOCAL, Error.Level.WARN,
							"unit data received from illegal unit : " + unitId + " ; unit data : " + aData);
				} else {
					// if (log.isInfoEnabled()) log.info("DataCollection:" + replyAddress + "
					// received from : " + unitId);
					result.put(unitId, aData);
					if (numberOfMembers <= result.size()) {
						// When the number of members defined in POLICY has been collected, end
						// immediately
							...
							dataConsumer.unregister(resUnregister -> {
								if (resUnregister.succeeded()) {
									...
									ErrorUtil.reportAndFail(vertx, Error.Category.FRAMEWORK, Error.Extent.LOCAL,
											Error.Level.ERROR, resUnregister.cause(), onComplete);
								}
							});
						}
					}
				...
				dataConsumer.unregister(resUnregister -> {
					...
					ErrorUtil.reportAndFail(vertx, Error.Category.FRAMEWORK, Error.Extent.LOCAL, Error.Level.ERROR, t,
							onComplete);
				});
				...
				if (res.succeeded()) {
					...
					DeliveryOptions options = new DeliveryOptions().addHeader("replyAddress", replyAddress)
							.addHeader("gridMasterUnitId", ApisConfig.unitId());
					// if (log.isInfoEnabled()) log.info("DataCollection:" + replyAddress + "
					// requested");
					...
					Long dataCollectionTimeoutMsec = PolicyKeeping.cache().getLong(DEFAULT_DATA_COLLECTION_TIMEOUT_MSEC,
							"gridMaster", "dataCollectionTimeoutMsec");
					// Set a timeout
					// タイムアウトを仕込む
					// The timeout duration is {@code POLICY.gridMaster.dataCollectionTimeoutMsec}
					// (default: {@link #DEFAULT_DATA_COLLECTION_TIMEOUT_MSEC}).
					// 待ち時間は {@code POLICY.gridMaster.dataCollectionTimeoutMsec} ( デフォルト値 {@link
					// #DEFAULT_DATA_COLLECTION_TIMEOUT_MSEC} ).
	 ...
	 * This uses a different set of data to that used in
	 * {@link jp.co.sony.csl.dcoes.apis.main.app.gridmaster.main_loop.DealExecution#voltageReferenceUnitId()}.
	 * 
